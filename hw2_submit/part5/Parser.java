...
public class Parser {
    ...
    //Each for loop will be put into a block { } 
    //each block has one iterator variable whose name is defined by  ITERATOR_NAME
    private static final String ITERATOR_NAME = "__FOR_ITER__";
    
    private Scan scanner;
    private CodeWriter writer;
    //Stack of blocks' variable.
	...
	private void factor() {
		...
		if(is(TK.LPAREN)){
			...
			parse_error("Illegal input \'" + tok.string + "\' @ line:" + tok.lineNumber);
		}
	...
	private void ffor(){
		mustbe(TK.BEGINFOR);
		writer.writeLn("{ int " + ITERATOR_NAME + " = 0;");
		writer.write("for(" + ITERATOR_NAME +" = ");
		expr();//initial value
		mustbe(TK.FORSEPERATOR);
    ...
    //Check if a variable is re-declared and print warning if it is the case. 
    ...
    //Check if a variable is declared, print error message and exit if it is the case
    ...
    //Check if a variable (having scope modifier) is declared, 
    //print error message and exit if it is the case
